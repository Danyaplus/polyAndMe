...
let pollTimer=null,lastSyncStamp=0;
const emojis=["😀","😂","🥰","😍","😊","😉","😎","😭","😡","👍","👎","❤️","🔥","🎉","🤔","😴","🙈","🙏","💯","✨","😘","🤝","👌","😅","🤣","😇","🥳","😜","🤍","💙","💜","🫶","👀","😱","🤦","🙃","😏","🤗","💬","📷"];
...
async function syncMessages(silent=true){
 try{
  const {data,error}=await db.from("chat_messages").select("*")
    .eq("room_id",cfg.roomId).order("created_at").limit(500);
  if(error)throw error;
  const fresh=data||[];
  const before=messages.length;
  const newestBefore=messages.at(-1)?.id;
  const newestAfter=fresh.at(-1)?.id;
  if(before!==fresh.length||newestBefore!==newestAfter||
     fresh.some((m,i)=>JSON.stringify(m)!==JSON.stringify(messages[i]))){
    const hadNew=fresh.some(m=>!messages.some(old=>old.id===m.id)&&m.sender_id!==me.user_id);
    messages=fresh;
    await render(ui.searchInput.value);
    if(hadNew&&!silent){notifySound();await markRead();}
  }
  lastSyncStamp=Date.now();
 }catch(e){
  if(!silent)ui.conn.textContent="Нет связи";
 }
}
function startPolling(){
 clearInterval(pollTimer);
 pollTimer=setInterval(()=>syncMessages(false),1000);
}
...
async function enter(){
 me=await membership();if(!me)return false;
 ui.setup.classList.add("hidden");ui.chat.classList.remove("hidden");
 await load();subscribe();subscribeCalls();startPolling();ui.input.focus();return true
}
async function session(){
 const{data}=await db.auth.getSession();
 if(data.session)return data.session;
 const{data:newData,error}=await db.auth.signInAnonymously();
 if(error)throw error;
...
async function send(){
 ...
 try{
  if(selectedFile)path=await upload(selectedFile);
  ...
  // Показываем своё сообщение сразу, не ожидая Realtime.
  if(created&&!messages.some(m=>m.id===created.id)){
  ...
  // Дополнительная сверка с базой через небольшую паузу.
  setTimeout(()=>syncMessages(true),350);
 }
 catch(e){
  console.error(e);
  toast(e.message||"Ошибка отправки");
  if(path)await db.storage.from("chat-files").remove([path]).catch(()=>{});
 }
 finally{
  setBusy(false);
  ui.input.focus();
...
function subscribe(){
 ...
 .on("postgres_changes",{event:"*",schema:"public",table:"chat_messages",filter:`room_id=eq.${cfg.roomId}`},async p=>{if(p.eventType==="INSERT"){
 if(!messages.some(m=>m.id===p.new.id)){
  messages.push(p.new);
  messages.sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
  await render(ui.searchInput.value);
 }
 if(p.new.sender_id!==me.user_id){notifySound();markRead()}
}else{const i=messages.findIndex(m=>m.id===p.new.id);if(i>=0)messages[i]=p.new;await render(ui.searchInput.value)}})
 ...
 .subscribe(async status=>{
  ui.conn.textContent=status==="SUBSCRIBED"?"В сети":"Подключение…";
  if(status==="SUBSCRIBED"){
   await channel.track({name:me.display_name,online_at:new Date().toISOString()});
...
ui.joinForm.onsubmit=async e=>{
 e.preventDefault();ui.joinError.textContent="";
 const enteredCode=ui.code.value.trim();
 try{
  await session();
  const{data,error}=await db.rpc("join_private_chat",{p_room_id:cfg.roomId,p_invite_code:enteredCode});
  if(error)throw error;
  if(!data)throw new Error("Неверный код");
  localStorage.setItem("privateChatInviteCode",enteredCode);
  await enter();
...
ui.logout.onclick=async()=>{
 if(confirm("Выйти из чата на этом устройстве?")){
  localStorage.removeItem("privateChatInviteCode");
  clearInterval(pollTimer);
...
async function start(){
 setupEmoji();setupDrop();updateSound();
 if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js").catch(()=>{});
 if(!okConfig()){ui.joinError.textContent="Проверь config.js";return}
 ...
 try{
  ...
  const savedCode=localStorage.getItem("privateChatInviteCode");
  if(savedCode){
   const{data,error}=await db.rpc("join_private_chat",{p_room_id:cfg.roomId,p_invite_code:savedCode});
   if(!error&&data&&await enter())return;
  }
...
window.addEventListener("focus",()=>syncMessages(false));
document.addEventListener("visibilitychange",()=>{if(!document.hidden)syncMessages(false)});
start();
})();